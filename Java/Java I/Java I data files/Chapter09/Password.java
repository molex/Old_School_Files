...
public class Password
{
	...
	public void set(String pswd) throws Exception
	{
		...
		verifyFormat(pswd);						//verify password was entered correctly
		...
		if(!pswdHistory.contains(encryptPswd))	//if pswd not in recently used list
		{
			if(pswdHistory.size() == maxHistory) //if list is at max size
				pswdHistory.remove(0);			 //remove oldest password from list
			...
			if(expired)								//if pswd is expired
				expired = false;					//reset to not expired
			
			if(autoExpires)							//if pswd autoexpires,
	...
	public void validate(String pswd) throws Exception
	{
		...
		if(!pswdHistory.isEmpty())		//at least one password entry in history
		{
	...
	private void verifyFormat(String pswd) throws Exception
	{
		...
		if(pswd.length() < MIN_SIZE)
			throw new Exception("password must be at least" + MIN_SIZE + "charecters in length");
		
		if(pswd.length() > MAX_SIZE)
			throw new Exception ("Password cannot be greater the " + MAX_SIZE + " charecters in length");
		
		// scan through password to find if at least 1 number us used.
		for(int i=0;i< pswd.length() && !numFound; ++i)
			if(Character.isDigit(pswd.charAt(i)))
				numFound = true;
		
		if(!numFound)
			throw new Exception("Password is invalid - must have at least one number");
		}
	...
	private String encrypt(String pswd)
	{
		...
		encryptPswd.reverse(); //reverse order of characters in password
