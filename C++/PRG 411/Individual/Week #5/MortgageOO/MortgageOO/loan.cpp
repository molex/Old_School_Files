...
do{
	...
	if(inputType == 1){
		//declare variables
		double newLoanAmount,newRate,newMonthlyInterestRate,newTerm,newNumMonths,loanYear,loanRate;
		double loanYears[3];
		double loanRates[3];
		int i = 0;
		Loan newLoan;

		ifstream inFile; //allows program to read from a file
		inFile.open("c:\\rateFile.txt",ios::in);//tells program which file to open

		//This is an error message in case the file doesn't open for some reason
		if (!inFile) {
		  cerr << "Can't open input file " << "c:\\rateFile.txt" << endl;
		  exit(1);
		}
		

		do{
			inFile >> loanYear >> loanRate;
			loanYears[i] = loanYear;
			loanRates[i] = loanRate;
			i++;
		}while (i<=2);
